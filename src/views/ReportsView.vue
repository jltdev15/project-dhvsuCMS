...
// Utilities for date formatting (always Asia/Manila)
...
const generateVisitsReport = async () => {
  try {
    ...
    console.log('All visits data:', visitsData)
    console.log('Date range:', { startDate, endDate })

    const filteredVisits = Object.entries(visitsData)
      .map(([id, visit]: [string, any]) => ({ id, ...visit }))
      .filter((visit: any) => {
        const timestamp = visit.timestamp || visit.visitDate || visit.createdAt
        if (!timestamp) {
          console.log(`Visit ${visit.id}: No timestamp found`)
          return false
        }
        
        console.log(`Visit ${visit.id}: timestamp = ${timestamp}`)
        
        // Parse the timestamp and handle timezone properly
        let visitDate: Date
        if (timestamp.includes('Z')) {
          // For UTC timestamps, we need to handle timezone conversion properly
          // Extract just the date part from the UTC timestamp to avoid timezone issues
          const datePart = timestamp.split('T')[0] // Get "2025-09-17" from "2025-09-17T20:05:00.000Z"
          visitDate = new Date(datePart + 'T00:00:00Z') // Create a UTC date at midnight
        } else {
          visitDate = new Date(timestamp)
        }
        console.log(`Visit ${visit.id}: parsed date = ${visitDate}`)
        ...
        console.log(`Visit ${visit.id}: visitDateOnly = ${visitDateOnly}, startDateOnly = ${startDateOnly}, endDateOnly = ${endDateOnly}`)
        
        const isInRange = visitDateOnly >= startDateOnly && visitDateOnly <= endDateOnly
        console.log(`Visit ${visit.id}: isInRange = ${isInRange}`)
    ...
    const reportContent = `
      ...
      <div class="footer">
        <p>Generated by PAMSU CMS on ${formatDateTime(new Date().toISOString())}</p>
      </div>
    `
